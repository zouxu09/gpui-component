...
use gpui_component::go_board::{
    BoardTheme, BoundedGoBoard, GhostStone, GhostStoneType, GoBoard, HeatData, Line, LineType, Marker, MarkerType, Vertex,
    VertexEventHandlers,
};
...
pub fn theming_examples() {
    ...
    let textured_theme = BoardTheme::default()
        .with_board_texture("assets/wood_grain.png".to_string())
        .with_stone_textures(
            Some("assets/black_stone.png".to_string()),
            Some("assets/white_stone.png".to_string())
        )
        .with_random_variation(true, 5.0);  // Random rotation up to 5 degrees
...
pub fn bounded_board_examples() {
    ...
    let _large_bounded = BoundedGoBoard::with_size(19, 19, 400.0, 300.0)
        .with_vertex_size_limits(5.0, 15.0);
